#!/usr/bin/env bash
# Experiment 10: Fixed Beta vs Adaptive Beta - Figure D.13
set -euo pipefail
...
TMPDIR="${TMPDIR:-/tmp/exp10}"
mkdir -p "$TMPDIR"

H03="$TMPDIR/hopper_beta03_seed${SEED}.sh"
mkcmd "$H03" 0 \
  "--env_is_gym=0 \\
...
H05="$TMPDIR/hopper_beta05_seed${SEED}.sh"
mkcmd "$H05" 1 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Hopper \\
...
H07="$TMPDIR/hopper_beta07_seed${SEED}.sh"
mkcmd "$H07" 2 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Hopper \\
...
H_ADAPT="$TMPDIR/hopper_adapt_seed${SEED}.sh"
mkcmd "$H_ADAPT" 3 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Hopper \\
...
A03="$TMPDIR/ant_beta03_seed${SEED}.sh"
mkcmd "$A03" 4 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Ant \\
...
A05="$TMPDIR/ant_beta05_seed${SEED}.sh"
mkcmd "$A05" 5 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Ant \\
...
A07="$TMPDIR/ant_beta07_seed${SEED}.sh"
mkcmd "$A07" 6 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Ant \\
...
A_ADAPT="$TMPDIR/ant_adapt_seed${SEED}.sh"
mkcmd "$A_ADAPT" 7 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Ant \\
...
new_win_run "h_beta03" "$H03" "tfboard/exp10_beta/hopper_beta03_seed${SEED}"
new_win_run "h_beta05" "$H05" "tfboard/exp10_beta/hopper_beta05_seed${SEED}"
new_win_run "h_beta07" "$H07" "tfboard/exp10_beta/hopper_beta07_seed${SEED}"
new_win_run "h_adapt" "$H_ADAPT" "tfboard/exp10_beta/hopper_adapt_seed${SEED}"
new_win_run "a_beta03" "$A03" "tfboard/exp10_beta/ant_beta03_seed${SEED}"
new_win_run "a_beta05" "$A05" "tfboard/exp10_beta/ant_beta05_seed${SEED}"
new_win_run "a_beta07" "$A07" "tfboard/exp10_beta/ant_beta07_seed${SEED}"
new_win_run "a_adapt" "$A_ADAPT" "tfboard/exp10_beta/ant_adapt_seed${SEED}"
