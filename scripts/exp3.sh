#!/usr/bin/env bash
# Experiment 3: GWIL Baseline (Default Setting) - Table 1, Figure D.1
set -euo pipefail
...
new_win_run "hopper" "$H" "tfboard/exp3_gwil/hopper_seed${SEED}"
new_win_run "ant" "$A" "tfboard/exp3_gwil/ant_seed${SEED}"
new_win_run "cheetah" "$C" "tfboard/exp3_gwil/cheetah_seed${SEED}"
new_win_run "lift" "$L" "tfboard/exp3_gwil/lift_seed${SEED}"
new_win_run "door" "$D" "tfboard/exp3_gwil/door_seed${SEED}"
new_win_run "wipe" "$W" "tfboard/exp3_gwil/wipe_seed${SEED}"
