#!/usr/bin/env bash
# Experiment 9: Psi Sensitivity Test - Figure D.14
set -euo pipefail
...
TMPDIR="${TMPDIR:-/tmp/exp9}"
mkdir -p "$TMPDIR"

H05="$TMPDIR/hopper_psi05_seed${SEED}.sh"
mkcmd "$H05" 0 \
  "--env_is_gym=0 \\
...
H09="$TMPDIR/hopper_psi09_seed${SEED}.sh"
mkcmd "$H09" 1 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Hopper \\
...
H099="$TMPDIR/hopper_psi099_seed${SEED}.sh"
mkcmd "$H099" 2 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Hopper \\
...
A05="$TMPDIR/ant_psi05_seed${SEED}.sh"
mkcmd "$A05" 3 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Ant \\
...
A09="$TMPDIR/ant_psi09_seed${SEED}.sh"
mkcmd "$A09" 4 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Ant \\
...
A099="$TMPDIR/ant_psi099_seed${SEED}.sh"
mkcmd "$A099" 5 \
  "--env_is_gym=0 \\
  --algorithm=avatar_dice \\
  --env_id=Ant \\
...
new_win_run "h_psi05" "$H05" "tfboard/exp9_psi/hopper_psi05_seed${SEED}"
new_win_run "h_psi09" "$H09" "tfboard/exp9_psi/hopper_psi09_seed${SEED}"
new_win_run "h_psi099" "$H099" "tfboard/exp9_psi/hopper_psi099_seed${SEED}"
new_win_run "a_psi05" "$A05" "tfboard/exp9_psi/ant_psi05_seed${SEED}"
new_win_run "a_psi09" "$A09" "tfboard/exp9_psi/ant_psi09_seed${SEED}"
new_win_run "a_psi099" "$A099" "tfboard/exp9_psi/ant_psi099_seed${SEED}"
