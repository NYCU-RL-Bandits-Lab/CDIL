#!/usr/bin/env bash
# Experiment 5: Ablation Study - w_tar only (DemoDICE) - Table 2, Figure D.2
set -euo pipefail

SESSION="exp5_demodice"
ATTACH=0
SEED=""
...
new_win_run "hopper" "$H" "tfboard/exp5_demodice/hopper_seed${SEED}"
new_win_run "ant" "$A" "tfboard/exp5_demodice/ant_seed${SEED}"
new_win_run "cheetah" "$C" "tfboard/exp5_demodice/cheetah_seed${SEED}"
new_win_run "lift" "$L" "tfboard/exp5_demodice/lift_seed${SEED}"
new_win_run "door" "$D" "tfboard/exp5_demodice/door_seed${SEED}"
new_win_run "wipe" "$W" "tfboard/exp5_demodice/wipe_seed${SEED}"
