#!/usr/bin/env bash
# Experiment 4: IGDF+IQLearn Baseline (Default Setting) - Table 1, Figure D.1
set -euo pipefail
...
new_win_run "hopper" "$H" "tfboard/exp4_igdf/hopper_seed${SEED}"
new_win_run "ant" "$A" "tfboard/exp4_igdf/ant_seed${SEED}"
new_win_run "cheetah" "$C" "tfboard/exp4_igdf/cheetah_seed${SEED}"
new_win_run "lift" "$L" "tfboard/exp4_igdf/lift_seed${SEED}"
new_win_run "door" "$D" "tfboard/exp4_igdf/door_seed${SEED}"
new_win_run "wipe" "$W" "tfboard/exp4_igdf/wipe_seed${SEED}"
