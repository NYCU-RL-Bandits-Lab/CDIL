...
class Avatar(nn.Module):
    ...
    def update(self, init_states, expert_states, expert_actions, expert_next_states,
               union_states, union_actions, union_next_states, union_init_indices, union_indices_half, union_indices, timestep, init_len, state_len):#,source_shift=None, source_scale=None):
        ...
        # print('asdasdasdasd', state_len, init_len)
        # return
        ...
        union_states = torch.cat([union_states[union_indices_half], union_states[state_len+union_indices_half]], dim=0)
        union_actions = torch.cat([union_actions[union_indices_half], union_actions[state_len+union_indices_half]], dim=0)
        union_next_states = torch.cat([union_next_states[union_indices_half], union_next_states[state_len+union_indices_half]], dim=0)
        ...
        if self.flow_in_decoder:
            state_mapping_output_1 = self.flow_model.g(self.decoder(union_states_1).double())[0].float()
            state_mapping_output_2 = self.flow_model.g(self.decoder(union_states_2).double())[0].float()
        state_mapping_output_1 = torch.cat([state_mapping_output_1, torch.zeros([state_mapping_output_1.shape[0], 1]).to(self.device)], -1)
        action_mapping_output_1 = self.action_decoder(union_inputs_1)
        state_mapping_output_2 = torch.cat([state_mapping_output_2, torch.zeros([state_mapping_output_2.shape[0], 1]).to(self.device)], -1)
        action_mapping_output_2 = self.action_decoder(union_inputs_2)
        if self.flow_in_decoder:
            action_mapping_output_1 = self.action_flow_model.g(self.action_decoder(union_inputs_1).double())[0].float()
            action_mapping_output_2 = self.action_flow_model.g(self.action_decoder(union_inputs_2).double())[0].float()
        src_union_inputs_1 = torch.cat([state_mapping_output_1, action_mapping_output_1], -1).float()
        src_union_inputs_2 = torch.cat([state_mapping_output_2, action_mapping_output_2], -1).float()
        ...
        union_adv_nu_1 = - union_cost_1.detach() + self.discount * union_next_nu_1 - union_nu_1

        init_nu_2 = self.critic(init_states_2)
        union_nu_2 = self.critic(union_states_2)
        union_next_nu_2 = self.critic(union_next_states_2)
        union_adv_nu_2 = - union_cost_2.detach() + self.discount * union_next_nu_2 - union_nu_2
        ...
        non_linear_loss = self.non_expert_regularization * torch.logsumexp(union_adv_nu_full / self.non_expert_regularization, dim=0)
        ...
        if self.flow_in_decoder:
            next_state_output_1 = self.flow_model.g(self.decoder(union_next_states_1).double())[0].float()
            next_state_output_2 = self.flow_model.g(self.decoder(union_next_states_2).double())[0].float()
        next_state_output_1 = torch.cat([next_state_output_1, torch.zeros([next_state_output_1.shape[0], 1]).to(self.device)], -1)
        next_action_output_1 = self.action_decoder(next_union_inputs_1)
        next_state_output_2 = torch.cat([next_state_output_2, torch.zeros([next_state_output_2.shape[0], 1]).to(self.device)], -1)
        next_action_output_2 = self.action_decoder(next_union_inputs_2)
        if self.flow_in_decoder:
            next_action_output_1 = self.action_flow_model.g(self.action_decoder(next_union_inputs_1).double())[0].float()
            next_action_output_2 = self.action_flow_model.g(self.action_decoder(next_union_inputs_2).double())[0].float()
        src_next_qvalue_1 = self.src_critic(torch.cat([next_state_output_1, next_action_output_1], -1))
        src_union_adv_nu_1 = - union_cost_1.detach() + self.discount*src_next_qvalue_1 - src_qvalue_1
        src_next_qvalue_2 = self.src_critic(torch.cat([next_state_output_2, next_action_output_2], -1))
        src_union_adv_nu_2 = - union_cost_2.detach() + self.discount*src_next_qvalue_2 - src_qvalue_2

        src_union_adv_nu_full = torch.cat([src_union_adv_nu_1, src_union_adv_nu_2], dim=0)    
        mapping_loss = torch.mean(src_union_adv_nu_full**2)
        
        # weighted BC
        weight_1 = torch.exp((union_adv_nu_1 - torch.max(union_adv_nu_1)) / self.non_expert_regularization).unsqueeze(1)
        weight_1 = weight_1 / weight_1.mean()
        weight_2 = torch.exp((union_adv_nu_2 - torch.max(union_adv_nu_2)) / self.non_expert_regularization).unsqueeze(1)
        weight_2 = weight_2 / weight_2.mean()

        src_union_adv_nu_1 = src_union_adv_nu_1.detach()
        src_weight_1 = torch.exp((src_union_adv_nu_1 - torch.max(src_union_adv_nu_1)) / self.non_expert_regularization).unsqueeze(1)
        src_weight_1 = src_weight_1 / src_weight_1.mean()
        src_union_adv_nu_2 = src_union_adv_nu_2.detach()
        src_weight_2 = torch.exp((src_union_adv_nu_2 - torch.max(src_union_adv_nu_2)) / self.non_expert_regularization).unsqueeze(1)
        ...
        if timestep % 10 == 0:
            with torch.no_grad():
                ...
                if self.flow_in_decoder:
                    all_state_output_1 = self.flow_model.g(self.decoder(all_union_states_1).double())[0].float()
                all_state_output_1 = torch.cat([all_state_output_1, torch.zeros([all_state_output_1.shape[0], 1]).to(self.device)], -1)
                all_next_state_output_1 = self.decoder(all_union_next_states_1)
                if self.flow_in_decoder:
                    all_next_state_output_1 = self.flow_model.g(self.decoder(all_union_next_states_1).double())[0].float()
                all_next_state_output_1 = torch.cat([all_next_state_output_1, torch.zeros([all_next_state_output_1.shape[0], 1]).to(self.device)], -1)
                
                all_input_1 = torch.cat([all_union_states_1, all_union_actions_1], -1)
                all_action_output_1 = self.action_decoder(all_input_1)
                if self.flow_in_decoder:
                    all_action_output_1 = self.action_flow_model.g(self.action_decoder(all_input_1).double())[0].float()
                all_src_input_1 = torch.cat([all_state_output_1, all_action_output_1], -1).float()

                all_union_cost_val_1 = self.cost(all_input_1)
                all_union_cost_1 = torch.log(1 / (all_union_cost_val_1 + EPS2) - 1 + EPS2)

                all_union_nu_1 = self.critic(all_union_states_1)
                all_union_next_nu_1 = self.critic(all_union_next_states_1)
                all_union_adv_nu_1 = - all_union_cost_1 + self.discount * all_union_next_nu_1 - all_union_nu_1

                next_all_input_1 = torch.cat([all_union_next_states_1, self.actor(all_union_next_states_1)[0]], -1)
                all_next_action_output_1 = self.action_decoder(next_all_input_1)
                if self.flow_in_decoder:
                    all_next_action_output_1 = self.action_flow_model.g(self.action_decoder(next_all_input_1).double())[0].float()
                all_src_next_input_1 = torch.cat([all_next_state_output_1, all_next_action_output_1], -1)

                all_src_qvalue_1 = self.src_critic(all_src_input_1)
                all_src_next_qvalue_1 = self.src_critic(all_src_next_input_1)
                all_src_union_adv_nu_1 = - all_union_cost_1 + self.discount * all_src_next_qvalue_1 - all_src_qvalue_1

                self.c1_1 = torch.abs(
                    torch.exp((all_src_union_adv_nu_1 - torch.max(all_src_union_adv_nu_1)) / self.non_expert_regularization) - 
                    torch.exp((all_union_adv_nu_1 - torch.max(all_union_adv_nu_1)) / self.non_expert_regularization)
                ).mean().item()
                if hasattr(self, 'prev_union_adv_nu_1'):
                    c2_1 = torch.abs(
                        torch.exp((all_union_adv_nu_1 - torch.max(all_union_adv_nu_1)) / self.non_expert_regularization) - 
                        torch.exp((self.prev_union_adv_nu_1 - torch.max(self.prev_union_adv_nu_1)) / self.non_expert_regularization)
                    ).mean().item()
                    self.c2_smooth_1 = 0.9 * self.c2_smooth_1 + 0.1 * c2_1 if hasattr(self, 'c2_smooth_1') else c2_1
                else:
                ...
                if self.flow_in_decoder:
                    all_state_output_2 = self.flow_model.g(self.decoder(all_union_states_2).double())[0].float()
                all_state_output_2 = torch.cat([all_state_output_2, torch.zeros([all_state_output_2.shape[0], 1]).to(self.device)], -1)
                all_next_state_output_2 = self.decoder(all_union_next_states_2)
                if self.flow_in_decoder:
                    all_next_state_output_2 = self.flow_model.g(self.decoder(all_union_next_states_2).double())[0].float()
                all_next_state_output_2 = torch.cat([all_next_state_output_2, torch.zeros([all_next_state_output_2.shape[0], 1]).to(self.device)], -1)
                
                all_input_2 = torch.cat([all_union_states_2, all_union_actions_2], -1)
                all_action_output_2 = self.action_decoder(all_input_2)
                if self.flow_in_decoder:
                    all_action_output_2 = self.action_flow_model.g(self.action_decoder(all_input_2).double())[0].float()
                all_src_input_2 = torch.cat([all_state_output_2, all_action_output_2], -1).float()

                all_union_cost_val_2 = self.cost(all_input_2)
                all_union_cost_2 = torch.log(1 / (all_union_cost_val_2 + EPS2) - 1 + EPS2)

                all_union_nu_2 = self.critic(all_union_states_2)
                all_union_next_nu_2 = self.critic(all_union_next_states_2)
                all_union_adv_nu_2 = - all_union_cost_2 + self.discount * all_union_next_nu_2 - all_union_nu_2

                next_all_input_2 = torch.cat([all_union_next_states_2, self.actor(all_union_next_states_2)[0]], -1)
                all_next_action_output_2 = self.action_decoder(next_all_input_2)
                if self.flow_in_decoder:
                    all_next_action_output_2 = self.action_flow_model.g(self.action_decoder(next_all_input_2).double())[0].float()
                all_src_next_input_2 = torch.cat([all_next_state_output_2, all_next_action_output_2], -1)

                all_src_qvalue_2 = self.src_critic(all_src_input_2)
                all_src_next_qvalue_2 = self.src_critic(all_src_next_input_2)
                all_src_union_adv_nu_2 = - all_union_cost_2 + self.discount * all_src_next_qvalue_2 - all_src_qvalue_2

                self.c1_2 = torch.abs(
                    torch.exp((all_src_union_adv_nu_2 - torch.max(all_src_union_adv_nu_2)) / self.non_expert_regularization) - 
                    torch.exp((all_union_adv_nu_2 - torch.max(all_union_adv_nu_2)) / self.non_expert_regularization)
                ).mean().item()
                if hasattr(self, 'prev_union_adv_nu_2'):
                    c2_2 = torch.abs(
                        torch.exp((all_union_adv_nu_2 - torch.max(all_union_adv_nu_2)) / self.non_expert_regularization) - 
                        torch.exp((self.prev_union_adv_nu_2 - torch.max(self.prev_union_adv_nu_2)) / self.non_expert_regularization)
                    ).mean().item()
                    self.c2_smooth_2 = 0.9 * self.c2_smooth_2 + 0.1 * c2_2 if hasattr(self, 'c2_smooth_2') else c2_2
                else:
        ...
        pi_loss = - torch.mean(
            (time_weight_decay_1 * src_weight_2.detach() + (1 - time_weight_decay_1) * weight_2.detach()) * self.actor.get_log_prob(union_states_2, union_actions_2)
            + (time_weight_decay_2 * src_weight_1.detach() + (1 - time_weight_decay_2) * weight_1.detach()) * self.actor.get_log_prob(union_states_1, union_actions_1)
        )/2.0 + l2_loss
        # pi_loss = - torch.mean(
